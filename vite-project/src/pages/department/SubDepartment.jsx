...
import { Pencil, Trash2, Layers, Plus, AlertCircle, X, Search } from "lucide-react";

const initialData = [
  { id: 1, name: "Domestic Sales", status: "Active" },
  { id: 2, name: "International Sales", status: "Active" },
  { id: 3, name: "Corporate Legal", status: "Active" },
  { id: 4, name: "Recruitment Team", status: "Active" },
  { id: 5, name: "Backend IT", status: "Active" },
  { id: 6, name: "Digital Marketing", status: "Active" },
];
...
export default function SubDepartment() {
  ...
  const saveDepartment = () => {
    ...
    if (current) {
      ...
      setList((prev) => [{ id: Date.now(), name: deptName, status }, ...prev]);
    }
    setModalOpen(false);
  };
    ...
    <div className="bg-[#f8fafc] min-h-screen p-4 md:p-8 pt-24">
      <div className="max-w-5xl mx-auto">
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-slate-50/50 border-b border-slate-100">
                ...
                <th className="px-6 py-4 text-[11px] font-black uppercase tracking-widest text-slate-400">Sub Department Name</th>
                ...
                <tr key={d.id} className="hover:bg-slate-50/50 transition-colors group">
                  ...
                  <td className="px-6 py-5">
                    <div className="flex justify-center gap-2">
                      <button onClick={() => onEdit(d)} className="p-2.5 bg-white border border-slate-100 text-blue-600 rounded-xl hover:border-blue-200 hover:shadow-sm transition-all shadow-sm shadow-slate-100"><Pencil size={16} /></button>
                      <button onClick={() => onDelete(d)} className="p-2.5 bg-white border border-slate-100 text-red-600 rounded-xl hover:border-red-200 hover:shadow-sm transition-all shadow-sm shadow-slate-100"><Trash2 size={16} /></button>
                    </div>
                  </td>
                </tr>
              ))}
        ...
        <div className="md:hidden space-y-4">
          {paginatedData.map((d, i) => (
            <div key={d.id} className="bg-white p-5 rounded-3xl border border-slate-100 shadow-sm relative overflow-hidden">
              ...
              <h3 className="font-bold text-slate-800 text-lg mb-5">{d.name}</h3>
              <div className="flex gap-2">
                <button onClick={() => onEdit(d)} className="flex-1 bg-slate-50 text-blue-600 font-bold py-3 rounded-2xl text-xs">Edit Detail</button>
                <button onClick={() => onDelete(d)} className="flex-1 bg-red-50 text-red-600 font-bold py-3 rounded-2xl text-xs">Remove</button>
              </div>
            </div>
          ))}
        </div>
            ...
            <div className="bg-white w-full max-w-md rounded-[2.5rem] shadow-2xl relative z-10 overflow-hidden animate-in zoom-in duration-200">
              <div className="bg-[#0a4d44] p-6 text-white flex justify-between items-center">
                <div>
                  ...
                  <p className="text-emerald-200 text-xs">Update your organization structure</p>
                </div>
                <button onClick={() => setModalOpen(false)} className="bg-white/10 hover:bg-white/20 p-2 rounded-full"><X size={20} /></button>
              </div>

              <div className="p-8 space-y-6">
                <div>
                  <label className="text-[10px] font-black uppercase text-slate-400 ml-1">Sub Department Name *</label>
                  <input
                ...
                <button
                  ...
                  className="w-full bg-[#0a4d44] text-white py-4 rounded-2xl font-bold shadow-lg shadow-emerald-900/10 hover:bg-slate-800 transition-all mt-4"
                >
                  Save Sub Department
                </button>
              </div>
        ...
        {deleteOpen && (
          <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
            ...
            <div className="bg-white w-full max-w-sm rounded-[2.5rem] p-8 relative z-10 text-center animate-in zoom-in duration-200">
              ...
              <p className="text-slate-500 text-sm mb-8">Are you sure you want to remove this record? This action cannot be reversed.</p>

              <div className="grid grid-cols-2 gap-3">
                <button onClick={() => setDeleteOpen(false)} className="py-4 rounded-2xl font-bold text-slate-400 hover:bg-slate-50 transition-colors">Cancel</button>
                <button onClick={confirmDelete} className="py-4 rounded-2xl font-bold bg-red-500 text-white shadow-lg shadow-red-200 active:scale-95 transition-all">Yes, Delete</button>
              </div>
            </div>
          </div>
        )}
